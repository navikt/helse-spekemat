...
data class Pølserad(
    ...
    fun fjernPølserTilBehandling() = this.copy(pølser = pølser.filterNot { it.erÅpen() }.toSet())

    fun nyPølserad(pølse: Pølse): Pølserad = pølse.nyRadFra(this)

    fun leggTilNyPølse(pølse: Pølse): Pølserad = this.copy(pølser = setOf(pølse) + pølser)
